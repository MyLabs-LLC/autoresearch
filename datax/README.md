...
**`corpus`** — real-world documents, discovered through the
[`superdoc-dev/docx-corpus`](https://huggingface.co/datasets/superdoc-dev/docx-corpus)
index: 736K `.docx` files pre-classified by type and topic, whose topic vocabulary
happens to include exactly `healthcare`, `finance` and `government`.

> **Every file comes from one host, and original provenance is not recoverable.**
> All 736,706 rows in the index point at `docxcorp.us/documents/<id>.docx` — Cloudflare
> R2 storage operated by SuperDoc (`superdoc.dev`), who also publish the index. The
> documents were harvested from the public web, but the index keeps no original-source
> URL: `url` is the mirror address and `filename` is usually unusable for tracing
> (`content`, `index.php`, `13462`). You cannot tell which site a given document came
> from, and `source.reference` in the manifest names the **mirror**, not the origin.
>
> **The `id` is not the hash of the file you receive.** The corpus documents it as
> "content-addressed by SHA-256"; measured over 24 downloads, only 1 had
> `sha256(bytes) == id`. Delivery is sound — repeated fetches are byte-identical and
> the MD5 matches the server's ETag every time — and the content is authentic (our
> extraction of a served file overlaps the corpus' own extracted text on 74 of 75
> distinct words). The `id` hashes some earlier artifact, so treat it as a join key,
> **not** an integrity check. datax verifies downloads against the ETag instead, which
> catches truncation and transit corruption (though not tampering — the same server
> supplies both the bytes and the ETag).
>
> The corpus is licensed ODC-BY 1.0, but that covers the database; the underlying
> documents are third-party works whose individual licences are unknown. Weigh that
> before training on or redistributing them. See *Notes and limits* below.
...
> return 403 to non-browser clients. A versioned, downloadable index is at least
> reproducible; a handwritten URL list is link-rot waiting to happen. The corpus' own
> topic is kept only as provenance — documents still go through the judge, so the two
> labels can be compared rather than one being trusted.
...
- **No real personal data is generated by this project.** The `nemotron` documents are
  synthetic by construction. The `corpus` documents are real files harvested from the
  public web and *may* contain real personal information — they are fetched by URL and
  not redistributed here.
- **The two sources have very different provenance quality, and it matters.** Nemotron
  documents are synthetic, CC-BY-4.0, and traceable to a specific dataset row. Corpus
  documents arrive through a single third-party mirror with no recoverable original
  source, under a database licence (ODC-BY 1.0) that says nothing about the individual
  documents. If you intend to publish or train on this corpus, that asymmetry is the
  thing to resolve first: you can find real PII in a corpus document and have no way to
  trace it to an owner, honour a deletion request, or verify its licence. The
  `source.synthetic` flag on every record exists so you can split on exactly this.
